...
function DetailCard({
  ...
  return (
    <div>
      {/* Stat card */}
      <motion.div
        className="bg-white rounded-2xl border border-border p-5 flex items-center gap-4 shadow-sm overflow-hidden cursor-pointer select-none relative"
        variants={fadeUp}
        initial="hidden"
        animate="visible"
        custom={index}
      ...
      >
        ...
        <div
          className={`rounded-xl p-3 bg-linear-to-br ${gradient} text-white shrink-0 shadow-sm`}
        >
          {icon}
        </div>
        <div className="min-w-0">
          ...
          {sub && (
            <p className="text-xs text-sidebar-text mt-0.5 truncate">{sub}</p>
          )}
        </div>
      </motion.div>
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-xl border border-border shadow-2xl shadow-slate-900/20 overflow-hidden w-85 max-w-[90vw]"
            >
              {/* Header */}
              <div
                className={`px-4 py-3 ${headerBg} flex items-center justify-between`}
              >
...
export default function DashboardPage() {
  ...
  const { data: deptData, isLoading: l1 } = useSWR("dashboard-dept", () =>
    departments.index(),
  );
  const { data: itemData, isLoading: l2 } = useSWR("dashboard-items", () =>
    items.index(),
  );
  const { data: empData, isLoading: l3 } = useSWR("dashboard-emp", () =>
    employees.index(),
  );
  const { data: assetData, isLoading: l4 } = useSWR("dashboard-assets", () =>
    itemAssets.index(),
  );
  const { data: stockData, isLoading: l5 } = useSWR("dashboard-stocks", () =>
    stocks.index(),
  );
  const { data: assignData, isLoading: l6 } = useSWR(
  ...
  const deptList =
    (deptData as { data?: { data?: unknown[] } })?.data?.data ?? [];
  const itemList =
    (itemData as { data?: { data?: unknown[] } })?.data?.data ?? [];
  const empList =
    (empData as { data?: { data?: unknown[] } })?.data?.data ?? [];
  const typedAssets = ((assetData as { data?: { data?: unknown[] } })?.data
    ?.data ?? []) as RawAsset[];
  const typedStocks = ((stockData as { data?: { data?: unknown[] } })?.data
    ?.data ?? []) as RawStock[];
  const typedAssignments = ((assignData as { data?: { data?: unknown[] } })
    ?.data?.data ?? []) as RawAssignment[];
  ...
  const computeAssetByCategory = (deptId?: number): DetailRow[] => {
    const src = deptId
      ? typedAssets.filter((a) => a.department_id === deptId)
      : typedAssets;
  ...
  /* ── Summary counts ── */
  const activeAssignCount = typedAssignments.filter(
    (a) => a.status === "active",
  ).length;
  const lowStockCount = typedStocks.filter(
    (s) => s.is_below_minimum && Number(s.quantity) > 0,
  ).length;
  const outOfStockCount = typedStocks.filter(
