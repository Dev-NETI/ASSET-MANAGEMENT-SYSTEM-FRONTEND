...
const useResource = ({ route }: ResourceOptions) => {
  const index = (): Promise<AxiosResponse> => axios.get(`${route}`);

  const show = (id: RouteParam): Promise<AxiosResponse> =>
    axios.get(`${route}/${id}`);

  const showWith2Parameter = (
    param1: RouteParam,
    param2: RouteParam,
  ): Promise<AxiosResponse> => axios.get(`${route}/${param1}/${param2}`);

  const showWith3Parameter = (
    param1: RouteParam,
    param2: RouteParam,
    param3: RouteParam,
  ...
  const destroy2Parameter = (
    id: RouteParam,
    id2: RouteParam,
  ): Promise<AxiosResponse> => axios.delete(`${route}/${id}/${id2}`);
